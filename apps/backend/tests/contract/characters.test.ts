...
describe('GET /api/characters', () => {
  ...
  const listCharacters = async (query = '') => {
    const req = new Request(`${charactersUrl}${query}`, { method: 'GET' });
    return app.fetch(req);
  };
  ...
  describe('正常系', () => {
    it('デフォルトのlimitで一覧を取得できること', async () => {
      ...
      const listRes = await listCharacters();
    ...
    it('limitを指定するとその件数に絞り込まれ、hasMoreがtrueになること', async () => {
      ...
      const listRes = await listCharacters('?limit=2');
      expect(listRes.status).toBe(200);
    ...
    it('offsetを指定すると続きのページを取得できること', async () => {
      ...
      const firstPageRes = await listCharacters('?limit=2&offset=0');
      const firstPage = (await firstPageRes.json()) as {
        data: any[];
        hasMore: boolean;
      };

      const secondPageRes = await listCharacters('?limit=2&offset=2');
    ...
    it('nameで名前を部分一致検索できること', async () => {
      await createCharacter({ ...minimalCharacterData, name: 'Alice' });
      await createCharacter({ ...minimalCharacterData, name: 'Bob' });
      await createCharacter({ ...minimalCharacterData, name: 'Alicia' });

      const listRes = await listCharacters('?name=ali');
      expect(listRes.status).toBe(200);

      const body = (await listRes.json()) as { data: any[]; hasMore: boolean };
      expect(body.data.map((c) => c.name).sort()).toEqual(['Alice', 'Alicia']);
    });
  });

  describe('異常系', () => {
    it('limitが範囲外の場合、400エラーを返すこと', async () => {
      const listRes = await listCharacters('?limit=1000');
      expect(listRes.status).toBe(400);
    });

    it('offsetが負数の場合、400エラーを返すこと', async () => {
      const listRes = await listCharacters('?offset=-1');
      expect(listRes.status).toBe(400);
    });
  });
});
